...
import 'package:current_wifi_data_channel/exception/wifi_connection_exception.dart';
import 'package:current_wifi_data_channel/exception/wrong_wifi_connection_exception.dart';
import 'package:current_wifi_data_channel/simple_wifi_utils.dart';
...
class CurrentWifiDataChannel extends DataChannel {
  /// Constructor of the class
  ///
  /// [identifier] The Channel identifier
  CurrentWifiDataChannel(super.identifier);
  ...
  Future<void> initReceiver(ChannelMetadata data) async {
    ...
    if (!(connectivityResults.contains(ConnectivityResult.wifi))) {
      debugPrint(
        "[CurrentWifiDataChannel:initReceiver]: Connecting to wifi: $networkName",
      );
      ...
      int remoteNetworkInt = SimpleWifiUtils.computeNetworkAsInt(
        data.address,
        data.apIdentifier,
      );
      int localNetworkInt = SimpleWifiUtils.computeNetworkAsInt(
        ipChecked,
        maskChecked,
    ...
    client!.listen((data) async {
      ...
      if (data.length >= 4 && messageLength == -1) {
        ...
        messageLength = ByteData.sublistView(
