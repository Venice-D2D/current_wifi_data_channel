...
import 'package:current_wifi_data_channel/exception/wrong_wifi_connection_exception.dart';
...
class SimpleWifiUtils {
    ...
    if (Platform.isAndroid) {
      try {
        final String result = await platform.invokeMethod(
          'connectToRegisteredNetwork',
          {'ssid': ssid},
        );
        debugPrint(
          "[SimpleWifiUtils::connectToRegisteredWifi] Android wifi connection result: $result",
        );
        if (result.contains(
          "[SimpleWifiUtils::connectToRegisteredWifi] Connected to",
        )) {
          connected = true;
        }
      ...
      final result = await Process.run('nmcli', ['connection', 'up', ssid]);
  ...
  static Future<String?> getActiveInterfaceByType(String interfaceType) async {
    ...
    ProcessResult nmcliCommandResult = await Process.run('nmcli', [
      '-t',
      '-f',
      'DEVICE,TYPE,STATE',
      'device',
    ...
    if (nmcliCommandResult.exitCode == 0) {
      String nmcliCommandResultStr = nmcliCommandResult.stdout
          .toString()
          .trim();
      ...
      activeInterface = devicesInfo.firstWhere(
        (deviceInfo) => deviceInfo.endsWith('$interfaceType:connected'),
        orElse: () => '',
      );
  ...
  static Future<Map<String, String>?> getIpAndMaskForActiveInterfaceByType(
    ...
    if (activeInterface != null) {
      ...
      if (nmcliCommandResult.exitCode == 0) {
        ...
        String activeInterfaceInfo = nmcliCommandResult.stdout
            ...
            .split('\n')
            .firstWhere(
              (info) => info.startsWith('IP4.ADDRESS'),
              orElse: () => '',
            );
  ...
  static int transformIpToInt(String ip) {
    ...
    if (ipParts.length == 4) {
      ipInt =
          (int.parse(ipParts[0]) << 24) +
          (int.parse(ipParts[1]) << 16) +
          (int.parse(ipParts[2]) << 8) +
          int.parse(
            ipParts[3],
          ); //part0*2^24 + part1*2^16 + part2*2^8 + part3*2^0
    }
    return ipInt;
  }
  ...
  static int computeNetworkAsInt(String ip, String mask) {
    ...
    debugPrint(
      "[SimpleWifiUtils::transformIpToInt] ip int& mask int: ${ipInt & maskInt}",
    );
